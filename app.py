    ...
    if st.button("🚀 启动 AI 路径全景演算", type="primary", use_container_width=True):
        if not client:
            ...
            with st.spinner("AI 导师正在演算路径，请稍候..."):
                ...
                【生成规则】：
                1. **区分校内与拓展**：必须将课程分为 '校内' (官方必修) 和 '拓展' (针对用户的竞赛/保研目标)。
                2. **强化关联性**：严禁出现孤立节点。基础课必须是进阶课的 source。形成清晰的先修路径。
                3. **拒绝冗余**：合并相似节点。例如不要同时出现“C++”和“程序设计基础”。
                4. **JSON 结构**：输出纯 JSON，包含 'nodes' (id, label, type, stage) 和 'edges' (source, target)。
                5. type 只能是 '校内' 或 '拓展'。stage 是 '大一', '大二', '大三', '目标'。
                """
                try:
                    ...
                    st.session_state.target_node = None
                    st.success("✅ 演算成功！已根据你的学段自动点亮前置技能。")
